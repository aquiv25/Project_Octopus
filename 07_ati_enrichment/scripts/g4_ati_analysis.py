#!/usr/bin/env python3
"""
G4 enrichment at A-to-I editing sites — O. bimaculoides v2
Data: GSE284564_Oct_editing.csv
G4:   g4_pattern.bed (pattern-based PQS predictions)

Usage: python3 g4_ati_analysis.py
"""
...
G4_FILE    = Path("/Users/dassagaripova/Downloads/project/g4_pattern.bed")
...
def load_g4():
    """Load G4 BED as dict of sorted numpy arrays (starts, ends) per chrom."""
    print("Loading G4 predictions …")
    data = defaultdict(lambda: {"s": [], "e": []})
    chrom_sizes = {}
    with open(str(G4_FILE), "rt") as f:
        for line in f:
    ...
    print(f"  {n:,} G4 regions on {len(arrays)} chromosomes")
    return arrays, chrom_sizes


def count_overlaps_fast(chroms, positions, g4, window=0):
    """Vectorised overlap count using cummax trick."""
    total = 0
    for c in np.unique(chroms):
        ...
        lo = pos - window
        hi = pos + window + 1
        idx = np.searchsorted(starts, hi, side="left")
        cummax_ends = np.maximum.accumulate(ends)
        mask = idx > 0
        has_overlap = np.zeros(len(pos), dtype=bool)
        if mask.any():
            has_overlap[mask] = cummax_ends[idx[mask] - 1] > lo[mask]
        total += int(has_overlap.sum())
    return total


def count_overlaps(sites_df, g4, window=0):
    chroms = sites_df["chr"].values
    positions = sites_df["pos"].values.astype(np.int32)
    return count_overlaps_fast(chroms, positions, g4, window)
...
def run_analysis(label, sites_df, g4, chrom_sizes):
    ...
    obs = {w: count_overlaps(sites_df, g4, w) for w in WINDOWS}
    for w in WINDOWS:
    ...
    for i in range(N_PERM):
        ...
        for w in WINDOWS:
            perm[w].append(count_overlaps(sh, g4, w))
...
def make_plot(all_results, all_perms):
    ...
    ax.set_title("G4 enrichment at A-to-I editing sites")
    ...
    for ax, (res, perm, label) in zip(axes[1:], pairs):
        ...
        ax.set_xlabel("Sites overlapping G4")
        ax.set_ylabel("Permutation count")
        ax.set_title(f"{label} sites  |  ±{w_show}bp")
        ax.legend(fontsize=9)

    plt.suptitle("O. bimaculoides  |  G4 × A-to-I editing enrichment",
                 fontsize=13, y=1.02)
    plt.tight_layout()
    out = SCRIPT_DIR / "g4_ati_enrichment.png"
